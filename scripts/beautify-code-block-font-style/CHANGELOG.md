...
### v0.5.2 (2026-06-11 01:43:50)

- 优化：复用 shared 样式注入和数组去重工具，减少本地重复实现

### v0.5.1 (2026-06-10 21:35:50)

- 优化：按站点拆分代码块选择器，统一生成字体样式
- 优化：抽离字体栈和样式生成逻辑，减少重复拼接
- 改进：样式注入支持幂等执行，并在页面结构未就绪时安全回退
- 改进：复用 package 元数据，避免版本、作者和描述重复维护
- 改进：同步 grant 声明，移除未使用的 GM_addStyle 权限
