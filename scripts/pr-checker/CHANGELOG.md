...
### v1.3.2 (2026-06-11 01:41:16)

1. 优化：复用 shared 挂载目标、JSON 数组解析和属性变化观察工具，减少重复边界逻辑。

### v1.3.1 (2026-06-10 22:45:47)

1. 优化：重构 PR 检查脚本结构，拆分初始化、弹窗、检查项缓存和目标分支检查逻辑。
2. 优化：将脚本样式拆分到独立 CSS 文件中维护。
3. 改进：复用 shared 的样式注入、DOM 创建和元素等待工具，减少重复代码。
4. 改进：增强重复初始化、缺失 DOM、异常缓存数据和 dialog 兼容场景的边界处理。

### v1.3.0 (2026-02-05 22:37:34)

1. 新增：在bitbucket中创建PR前，检查目标分支是否为非法分支。
